...
public class ResidentService {
	...
	public int getResidentIdByUserId(int userId) {
	    Optional<Resident> residentOptional = residentRepository.findByUserId(userId);
	        return residentOptional.get().getId();
	    }

	/*public Resident getResident(int residentId)throws InvalidIdException {
		// TODO Auto-generated method stub
		Optional<Resident> optional =residentRepository.findByResidentId("residentId");
		if(!optional.isPresent())
		{
			throw new InvalidIdException("ResidentId invalid");
		}
		return optional.get();
	}*/
}